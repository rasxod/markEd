...
(function ($) {
		...
		html += '<div class="md-toolbar">';
			html += '<div class="btn-toolbar">';
				...
				html += '<div class="btn-group">';
					...
					if (options.imageBrows === true) {
						html += '<div data-mdtooltip="tooltip" title="' + options.label.btnBrows + '" class="md-btn btn btn-sm btn-default" data-btn="imgBrowser"><span class="glyphicon glyphicon-record"></span></div>';
					}
				html += '</div>'; // .btn-group
	...
	function _imageBrowser (data) {
		...
		$.ajax({
			...
			success:function(data){
				$('.md-brows').empty().append('<button type="button" class="close" aria-hidden="true">&times;</button>').show();
				jQuery.each(data, function( i, bval ) {
					$('.md-brows').append(bval);
				});
			}
		});
	}
	var methods = {
		init: function (options) {
			...
			container.find('.md-brows').on('click', '.close', function(){
				$('.md-brows').hide();
			});
			...
			container.find('.md-btn').click(function () {
				...
				} else if (btnType === 'imgBrowser') {
					_imageBrowser(defaults.uploadDir);
